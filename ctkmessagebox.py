...
class CTkMessagebox(ctk.CTkToplevel):
    ...
    def __init__(
        ...
        self.button_border_width = (
            button_border_width if button_border_width < 6 else 5
        )  # ! add
        ...
        if button_border_color == "default":
            self.button_border_color = self._apply_appearance_mode(
        ...
        if self.info.winfo_reqheight() > self.height / 2:
            height_offset = int(
                (self.info.winfo_reqheight()) - (self.height / 2) + self.height
            )
            self.geometry(f"{self.width}x{height_offset}")
        ...
        if button_width:
            ...
            span = 1
        self.button_1.grid(
            row=2, column=columns[0], columnspan=span, sticky="ew", padx=(0, 10), pady=6
        )
        if option_2:
            self.button_2.grid(
                row=2, column=columns[1], columnspan=span, sticky="ew", padx=10, pady=6
            )
        if option_3:
            self.button_3.grid(
                row=2,
